		...
		<div class="row">
			...
			<div class="col-lg-2 col-md-2  col-xs-12">
				<div class="alert alert-warning">
					<spring:message code="page.blog.temporary.tip" />
				</div>
		...
		$.get(url,{},function callBack(data){
			...
			if(data.success){
				...
				for(var i=0;i<rows.length;i++){
					var row = rows[i];
					if(row.id == currentCategory){
						current = '<option value="'+row.id+'"><spring:message code="page.item.current" />:'+row.name+'</option>';
					}
					$("#categorys").append('<option value="'+row.id+'">'+row.name+'</option>');
				}
				if(current != ""){
					$("#categorys").prepend(current);
				}
			}
		});
	}
