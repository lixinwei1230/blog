...
function getToReadMessageCount() {
	if (isGetToReadMessageCount) {
		$.ajax({
			type : "GET",
			url : contextPath
					+ "/my/message/receive/getToReadMessageCount?sources=1",
			dataType : "json",
		...
		.ready(
				function() {
					if (getCookie("WEBP_SUPPORT") == null) {
						hasWebP().then(
								function() {
									setRootCookie("WEBP_SUPPORT", "true", 24
											* 60 * 60 * 1000 * 30)
								},
								function() {
									setRootCookie("WEBP_SUPPORT", "false", 24
											* 60 * 60 * 1000 * 30);
								});
					}
					...
					$("img.avatar").error(
							function() {
								var me = $(this);
								me.attr("src", staticSourcePrefix
										+ "/imgs/guest_160.png")
							});
					...
					$(document)
							.on(
									...
									function() {
										...
										var loading = $('<img src="'
												+ staticSourcePrefix
												+ '/imgs/loading-64.gif" class="img-responsive"/>');
										me.parent().hide().after(loading);
										var img = $(me.find("img")[0]);
										img.attr("src", $(this).attr(
												"data-play"));
...
function setRootCookie(name, value, time) {
	...
	document.cookie = name + "=" + escape(value) + ";expires="
			+ exp.toGMTString() + ";path=/;domain=" + getRootDomain();
}
// 读取cookies
